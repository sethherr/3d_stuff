...
directly into the underside of the plate at center; the cradle-side prong is
tapped M5 (×0.8) and carries a truncated cone on its outboard face (as on a real
GoPro clevis) for extra thread engagement. Over each cup an open tie
channel is cut
...
tunnels), and the GoPro clevis — is one fused manifold solid (`solids: 1`).
